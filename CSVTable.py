...
class CSVTable:
    ...
    def __add_row__(self, row):
        ...
        for k, index in defined_indexes.items():
    ...
    def __load__(self):
        ...
        self.__keys_added__ = []
        for k, index in given_indexes.items():
    ...
    def __get_column_names__(self):
        ...
        for k, column in column_list.items():
    ...
    def __get_access_path__(self, fields):
        ...
        if self.__description__.indexes is None or fields is None:
            return None, None
        ...
        for k, index in self.__description__.indexes.items():
            match = True
            for col in index.column_names:
                if col not in fields.keys(): # fields passed as a dictionary
                    match = False
                    break

            if match:
                key = self.__get_key__(index, fields)
                if selected_index is None:
                    selected_index = index
                    count = len(self.__indexes__[index.index_name][key])
                else:
                    if count > len(self.__indexes__[index.index_name][key]):
                        count = len(self.__indexes__[index.index_name][key])
                        selected_index = index
    ...
    def __find_by_template_scan__(self, t, fields=None):
        ...
        for r in self.__rows__:
            if self.matches_template(r, t):
                result.append(r)
        return self.project(result, fields)
    ...
    def __find_by_template_index__(self, t, idx_name, fields=None):
        ...
        rows = self.__indexes__[idx_name][self.__get_key__(self.__description__.indexes[idx_name], t)]
    ...
    def __smart_join__(self, right_r, on_fields, where_template=None, project_fields=None):
        ...
        Optimization 2: create index on on_fields for the right table, so for each row in the left table, we can find 
        the matching key of this index in O(1) time. And then generate the resulting rows accordingly.
        """

        left_r = self
        left_rows = left_r.__find_by_template__(self.__get_sub_where_template__(where_template)) # filter before join
        right_rows = right_r.__find_by_template__(right_r.__get_sub_where_template__(where_template))

        # create index on on_fields for the right table
        on_field_index = CSVCatalog.IndexDefinition("on_field_index", "INDEX", on_fields)
        idx = {}
        for r in right_rows:
            key = self.__get_key__(on_field_index, r)
        ...
        left_rows_processed = 0
        for lr in left_rows:
            key = self.__get_key__(on_field_index, lr)
            for rr in idx[key]: # use the index created above
                new_r = {**lr, **rr}
                result_rows.append(new_r)
        ...
        result_rows = self.project(result_rows, project_fields)
        final_table = self.__table_from_rows__("Filtered JOIN(" + self.__table_name__ + "," + right_r.__table_name__ + ")", result_rows)
