...
class TableDefinition:
    ...
    def load_columns(self):
        ...
        self.columns = {} # a dict {column_name : ColumnDefinition}
        ...
        for r in res:
            if r['not_null'] == 1:
                ...
                r['not_null'] = False
            self.columns[r['column_name']] = ColumnDefinition(r['column_name'], r['type'], r['not_null'])
    ...
    def load_indexes(self):
        ...
        d = {}
        for r in res:
            if r['index_name'] in d.keys():
                d[r['index_name']][2][r['index_order']] = r['column_name']
            else:
                d[r['index_name']] = [r['index_name'], r['type'], {r['index_order'] : r['column_name']}]

        self.indexes = {}
        for k, v in d.items():
            self.indexes[k] = IndexDefinition(v[0], v[1], [v for k, v in sorted(v[2].items())])
    ...
    def load_core_definition(self):
        ...
        res = run_q(self.cnx, q, (self.table_name), fetch=True) # list of dicts
        if len(res) > 0:
    ...
    def save_core_definition(self):
        ...
        q = "insert into csvtables values(%s, %s) on duplicate key update path=%s"
        result = run_q(self.cnx, q, (self.table_name, self.file_name, self.file_name), fetch=True)
    ...
    def drop_col_in_sql(self, cn):
        ...
        """
        # ************************ TO DO ***************************
        # can't delete this column if there are indexes that depend on it
        q = "delete from csvcolumns where table_name=%s and column_name=%s and " \
            "column_name not in (select column_name from csvindexes where table_name=%s)"
        run_q(self.cnx, q, (self.table_name, cn, self.table_name), fetch=False)
    ...
    def to_json(self):
        ...
        if self.indexes is not None:
            ...
            result['indexes'] = []
            for k, idx in self.indexes.items(): # add indentation
                result['indexes'].append(idx.to_json())
        return result


    def save_index_definition(self, index_name, columns, type):
        """
        ...
        :param columns: columns the index is defined on
        ...
        for i in range(0, len(columns)):
            v = (self.table_name, columns[i], type, index_name, str(i))
    ...
    def define_index(self, index_name, columns, type="INDEX"):
        ...
        """
        # ************************ TO DO ***************************
        if index_name in self.indexes.keys():
    ...
    def get_index(self, ind_name):
        ...
        """
        # ************************ TO DO ***************************
        if ind_name in self.indexes.keys():
    ...
    def drop_index(self, index_name):
        ...
        # ************************ TO DO ***************************
        if index_name in self.indexes.keys():
            q = "delete from csvindexes where table_name=%s and index_name=%s"
            run_q(self.cnx, q, (self.table_name, index_name), fetch=False)
...
class CSVCatalog:
    ...
    def __init__(self, dbhost="localhost", dbport=3306,
                  dbuser="root", dbpw="222222",  db="CSVCatalog",debug_mode=None):
